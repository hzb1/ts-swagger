...
const ApiInfo = ({api, codeMap}: ApiInfoProps) => {
  ...
  return <>
    <div
      className={'flex flex-col box-border w-full relative grow mx-auto [contain:inline-size] pb-20'}>
      <header className={'relative'}>
        ...
        <div className={'flex flex-col sm:flex-row items-start sm:items-center relative gap-2 min-w-0'}>
          <h1
            className={'text-2xl sm:text-3xl text-gray-900 tracking-tight dark:text-gray-200 [overflow-wrap:anywhere] break-all font-semibold'}>
            {title}
          </h1>
        </div>
      ...
      <div className={'flex flex-col gap-8 isolate'}>
        <div className={'mt-6 flex w-full flex-col space-y-4'}>
          <div
            className={'flex w-full flex-col bg-background-light dark:bg-background-dark border-standard rounded-2xl p-1.5'}>
            <div className="flex items-center space-x-1.5">
              <div
                className="relative flex-1 flex gap-2 min-w-0 rounded-xl items-center cursor-pointer p-1.5 pr-0 overflow-hidden border-standard">
                <div
                  className="method-pill rounded-lg font-bold px-1.5 py-0.5 text-sm leading-5 bg-blue-400/20 dark:bg-blue-400/20 text-blue-700 dark:text-blue-400">
                  {api.method}
                </div>
                <div className="flex items-center space-x-2 overflow-x-auto flex-1 no-scrollbar">
                  <div className="group flex items-center flex-1 gap-0.5 font-mono pr-1.5">
                    <div
                      className="absolute right-0 p-2 bg-background-light dark:bg-background-dark rounded-lg hidden group-hover:block">
                      <svg className="w-4 h-4 bg-gray-400 dark:bg-white/30"
                           style={{
                             maskImage: 'url(&quot;https://d3gk2c5xim1je2.cloudfront.net/v7.1.0/regular/clone.svg&quot;)',
              ...
              <button aria-label="试一试"
                      ...
                      data-testid="try-it-button"><span>试一试</span>
                <svg className="w-3 h-3 bg-white"
                     style={{
